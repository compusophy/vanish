...
async function runCheckDeployment(args = {}) {
  if (!vercelConfigured()) {
    const status = vercelConfigStatus();
    return {
      error:
        'vercel credentials are not configured, so build results are invisible to this agent. ' +
        'commits still deploy, but failures cannot be read back.',
  ...
  try {
    if (args.wait) {
      ...
      if (!sha) {
        ...
        sha = latest?.sha || null;
        if (!sha) return await deploymentReport(latest?.id || null, { logLines });
      }
      return await waitForDeployment(sha, { timeoutMs: 45000, pollMs: 3000 });
    }
    return await deploymentReport(args.sha || null, { logLines });
  } catch (err) {
    return { error: `vercel api failed: ${err.message}` };
  }
}
  ...
  switch (name) {
    ...
    case 'deploy_vercel': {
      ...
      return {
        ...
        last_commit: ctx.lastCommit || null,
        latest_deployment: vercelConfigured()
          ? await runCheckDeployment({ sha: ctx.lastCommit?.sha })
          : null
      };
    }

    case 'check_deployment':
      return await runCheckDeployment({
        ...args,
        sha: args.sha || ctx.lastCommit?.sha
      });
...
async function executeLocalTool(name, args) {
  switch (name) {
    ...
    case 'check_deployment':
      return await runCheckDeployment(args);
