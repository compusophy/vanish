...
async function executeGithubTool(name, args, ctx) {
  switch (name) {
    ...
    case 'deploy_vercel': {
      ...
      return {
        success: true,
        note: `this project is connected to github with continuous deployment. every commit to ${ctx.branch} deploys automatically, so git_commit is the deploy step. set VERCEL_DEPLOY_HOOK_URL to force a rebuild without a commit.`,
        last_commit: ctx.lastCommit || null
      };
    }
