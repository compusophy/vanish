...
// commit statuses are labelled "Vercel - <project name>", so matching them
// needs the name rather than the id. VERCEL_BRANCH_URL is always
// "<project>-git-<branch>-<team>.vercel.app" on a git deployment, which makes
// the name derivable with no extra configuration.
export function projectName() {
  if (process.env.VERCEL_PROJECT_NAME) return process.env.VERCEL_PROJECT_NAME;
  const branchUrl = process.env.VERCEL_BRANCH_URL || '';
  const idx = branchUrl.indexOf('-git-');
  if (idx > 0) return branchUrl.slice(0, idx);
  return null;
}
